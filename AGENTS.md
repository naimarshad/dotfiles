...
- **Compositor + shell:** `niri` (`extra`, 26.04) with `noctalia` (`extra`, 5.0.1) as the desktop shell; `greetd` + `noctalia-greeter` (AUR) is the display manager (`/etc/greetd/config.toml` needs `[terminal]` `vt = 1`); GNOME stays installed as a fallback session. DankMaterialShell (`dms-shell-niri`, `extra`) is an alternative shell picked at the greeter; niri + Noctalia is the default and the two shells do not run together.
