...
- **Stow packages actually used here:** `ghostty zsh niri nvim tmux k9s btop arch-update`. `noctalia` is a stow package name but is **not** stowed (SOPS-encrypted, decrypted to `~/.local/state/noctalia/settings.toml`). `hypr`, `fish`, `starship` are retained but dormant.
- **Updates:** `arch-update` (AUR) is the update path (notifier + updater + post-update cleanup: orphans, `paccache`, `.pacnew`, reboot check). The AUR package ships no `arch-update-tray.service`, only `arch-update.service`/`arch-update.timer` (the timer-driven check, deliberately *not* enabled since the tray covers it). The tray applet is spawned by niri itself: `niri/.config/niri/autostart.kdl` runs `arch-update --tray` and shows in Noctalia's `tray` bar widget. It has to wait for Noctalia's `org.kde.StatusNotifierWatcher` to be on the session bus before registering, so the autostart line polls for it (`busctl --user list | grep -q org.kde.StatusNotifierWatcher`) before `exec`ing `arch-update --tray`; without that wait it registers too early, gets no watcher, and exits silently at boot. Config is the `arch-update` stow package (`~/.config/arch-update/arch-update.conf`). `informant` stays as the pacman-level news gate on top of it. `arch-update`'s orphan step offers to remove any dependency-only leaf every run, so `go` and `jq` must stay `pacman -D --asexplicit`.
