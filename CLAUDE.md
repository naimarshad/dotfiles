...
- **Shell:** `noctalia` from `extra` (5.0.1), `/usr/bin/noctalia`. `autostart.kdl`'s last line runs `niri/.config/niri/start-shell.sh`, which brings up Noctalia unless the `$XDG_RUNTIME_DIR/niri-shell.dms` marker is set. Noctalia 5 IPC is `noctalia msg <verb>`; Noctalia 4's `qs -c noctalia-shell ipc call ...` is gone and `qs` is not installed. The `binds.kdl` and `autostart.kdl` IPC calls were migrated 4→5 (calendar and now-playing are `control-center` tabs now, not standalone panels). The legacy AUR `noctalia-qs` / `noctalia-shell` packages are v4; do not install them.
- **Alternative shell:** DankMaterialShell (`dms-shell-niri` + `dgop` + `matugen`, all `extra`) is a second option. The greeter session "Niri (DankMaterialShell)" (`/usr/local/share/wayland-sessions/niri-dms.desktop` → `/usr/local/bin/niri-dms-session`) drops the marker, so `start-shell.sh` runs `dms run` instead of Noctalia. niri + Noctalia stays the default (`--session niri` in greetd config). The two shells cannot run on screen together. `binds.kdl`'s IPC keys call `niri/.config/niri/shell.sh <action>`, which routes to `noctalia msg ...` or `dms ipc call ...` depending on which shell `pgrep` finds, so the same keymap works in both sessions. DMS config is `~/.config/DankMaterialShell/` (not stowed, not tracked). See runbook Step 22g. Theming caveat: DMS uses the real `matugen` binary with its own templates; don't point it at the `noctalia`-generated theme files or the two generators fight.
...
- **Stow packages actually used here:** `ghostty zsh niri nvim tmux k9s btop arch-update zed`. `zed` holds `settings.json`, `keymap.json` and `tasks.json` (vim mode, space-leader chords mirroring the Neovim README, Claude Code over ACP); `~/.config/zed/themes/` is Noctalia-generated and untracked. Its README lists the binaries Zed expects on PATH. File icons come from `zed-noctalia-icons/` at the repo root, a Zed icon-theme extension (not a stow package) hand-drawn in the Noctalia light palette; it needs **Install Dev Extension** once per machine, and its colours are a hand-maintained palette copy that a desktop theme change does not update. `noctalia` is a stow package name but is **not** stowed (SOPS-encrypted, decrypted to `~/.local/state/noctalia/settings.toml`). `hypr`, `fish`, `starship` are retained but dormant.
- **Updates:** `arch-update` (AUR) is the update path (notifier + updater + post-update cleanup: orphans, `paccache`, `.pacnew`, reboot check); `arch-update.timer` is deliberately not enabled, the tray covers periodic checks instead. The tray applet is spawned by niri: `niri/.config/niri/autostart.kdl` polls for Noctalia's `org.kde.StatusNotifierWatcher` on the session bus, then execs `arch-update --tray`, which shows in Noctalia's `tray` bar widget. Config is the `arch-update` stow package (`~/.config/arch-update/arch-update.conf`). `informant` stays as the pacman-level news gate on top of it. `arch-update`'s orphan step offers to remove any dependency-only leaf every run, so `go` and `jq` must stay `pacman -D --asexplicit`.
