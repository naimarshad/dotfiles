...
- **Updates:** `arch-update` (AUR) is the update path (notifier + updater + post-update cleanup: orphans, `paccache`, `.pacnew`, reboot check). `arch-update-tray.service` runs `--user` and shows in Noctalia's `tray` bar widget; the `arch-update.timer` is deliberately *not* enabled (either/or, not both). Config is the `arch-update` stow package (`~/.config/arch-update/arch-update.conf`). `informant` stays as the pacman-level news gate on top of it. `arch-update`'s orphan step offers to remove any dependency-only leaf every run, so `go` and `jq` must stay `pacman -D --asexplicit`.
