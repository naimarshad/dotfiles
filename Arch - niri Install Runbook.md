...
> - The tray applet is a niri `spawn-at-startup` line in `autostart.kdl`, not a systemd unit: `arch-update --tray` needs Noctalia's `org.kde.StatusNotifierWatcher` on the session bus to register its icon, so the line polls for it with `busctl --user list | grep -q org.kde.StatusNotifierWatcher` before `exec`ing `arch-update --tray`. Keep it as **either** that tray line **or** `arch-update.timer` enabled, never both, or updates get checked (and notified) twice.
...
- [ ] `arch-update` + `libnotify` from AUR/`extra` · `arch-update --check` runs · niri `autostart.kdl` spawns the tray, waiting for `StatusNotifierWatcher` (not the timer) · tray icon visible in Noctalia's `tray` widget
