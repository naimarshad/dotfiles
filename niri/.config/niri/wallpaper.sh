...
#   2. echo "YOUR_ACCESS_KEY" > ~/.config/niri/unsplash-key
#
# Log: ~/.cache/wallpapers/wallpaper.log

CACHE_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/wallpapers"
KEY_FILE="$HOME/.config/niri/unsplash-key"
...
fetch_and_set() {
    ...
    local raw_url
    raw_url=$(echo "$response" | python3 -c \
        "import sys,json; d=json.load(sys.stdin); print(d['urls']['raw'])" 2>/dev/null)
